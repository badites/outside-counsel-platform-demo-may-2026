...
const statusConfig: Record<InvitationStatus, { icon: typeof Clock; label: string; className: string }> = {
  ...
  SUBMITTED: { icon: FileText, label: "Response received", className: "text-teal-600 bg-teal-50 border-teal-200" },
  ...
  SELECTED: { icon: CheckCircle2, label: "Selected", className: "text-teal-700 bg-teal-100 border-teal-300" },
              ...
              <div className="flex items-center gap-2">
                {inv.status === "INVITED" && (
                  <Link
                    ...
                    className="inline-flex items-center gap-1 rounded px-2 py-0.5 text-[10px] font-medium text-teal-700 hover:bg-teal-50"
                  >
                    <Pencil size={10} />
                    Enter response
                  </Link>
